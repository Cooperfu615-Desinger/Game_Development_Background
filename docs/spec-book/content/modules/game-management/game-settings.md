...
> 本頁不管理 RTP、賠率表、程式版本、素材或代理商／商戶／會員層級設定。現行表單可直接改動 mock，但正式系統需草稿、驗證、審核、版本與 audit；畫面值不代表已生效。

## 1. 目的與責任邊界

- 管理 Provider 共用維護週期模板、投注／派彩限制模板與遊戲基礎選項。
- 允許單款遊戲引用已核准模板；不把模板本身當成已發布遊戲設定。
- 影響 Production 的異動必須進入審核與發布組合，不直接覆寫生效值。
...
- 模板 Draft 狀態至少分 draft、pending_review、approved、active、archived；正式 enum 依 `TBD-DOM-003`。
- 已被生效發布組合引用的版本不可原地修改；需複製新版本。
...
欄位至少含模板 ID、名稱、Provider 點數規則版本、最小投注、最大投注、單 Round／期間派彩上限（若適用）、scope、遊戲類型／遊戲、版本、狀態及時間。

- 必須滿足 `0 ≤ min_bet ≤ max_bet`，派彩上限與遊戲數值模型相容。
- 主值是 Provider 點數，若顯示 USDT 必須使用已指定換算規則版本；禁止把外部法幣當 Provider 限紅主值。
- 修改 active 模板需複製新版本；發布組合引用精確版本。
...
- Production 變更需理由、差異預覽、最新 version 與 allowed action；正式核准策略依 `TBD-SEC-003`。
...
| 送審／狀態轉換 | approval ID、allowed actions、new version、audit event。 |
...
驗收條件：設定與數值／版本／素材分離；點數為限紅主值；active 不可原地改；維護不阻斷既有 Round；Production 具草稿／驗證／核准／audit 骨架；遊戲套用保存版本 snapshot；Loading／error／conflict／Forbidden 可驗收。
...
設定 schema、版本、驗證、遊戲套用、API、Production 核准與權限核准並通過驗收後，才可改為 Confirmed。
