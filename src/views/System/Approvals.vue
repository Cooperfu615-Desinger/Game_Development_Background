...
onMounted(async () => {
  try {
    const data = await api.get<ApprovalRow[]>('/api/system/v2/approvals');
    approvalRows.splice(0, approvalRows.length, ...data);
  } finally {
    loading.value = false;
  }
});
...
<template>
  <div class="page-stack reports-page system-approvals-page">
    ...
    <FilterCard title="查詢條件" description="查詢審核單、模組、申請人、狀態、優先級與申請時間。">
