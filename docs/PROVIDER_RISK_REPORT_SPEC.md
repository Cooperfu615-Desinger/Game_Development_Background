...
若由風控告警／處理頁以 `risk_event_id` 導入本頁，報表必須解析完整識別碼，找到事件後自動切換至事件所屬環境並開啟對應 Risk Event 詳情；同頁 query 變更也必須重新定位。找不到指定事件時顯示明確錯誤，不得靜默開啟其他事件或預設詳情。由 Alert → Risk Event → Alert 往返時，`risk_event_id` 與 `alert_id` 必須保持一致。
