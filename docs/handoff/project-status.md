...
> 狀態日期：2026-08-08
> 目前分支：`main`
> 文件狀態：Provider Portal 第一至四階段導覽、頁面原型與文件地圖已建立；正式 API、權限與後端資料契約待確認
...
目前程式仍保留早期三 Portal legacy code，但主要工作區已切換為 Provider Portal 目標導覽，並完成 32 個導覽內容頁的 route 與第一至三階段前端原型；其中 12 個頁面使用共用 Provider Placeholder mock blueprint。它目前適合用於規格討論與畫面確認，不是正式營運系統。
...
1. 完成第五階段整體驗證：主要 route、已完成頁面、手機版、console 與導覽文件一致性檢查。
2. 與 GGAP / 後端對接團隊確認 Provider API、身份、冪等、錯誤與權限契約。
3. 將 Game Round 頁面接上正式資料，確認點數 / USDT、GGR 與報表指標的正式定義。
4. 將財務總覽與代理商 × 遊戲彙總接上 Game Round 聚合資料，確認日期篩選、不重複玩家數、GGR 與 USDT 規則。
5. 依正式資料契約補齊財務報表權限、資料版本與查詢效能限制。
6. 將遊戲大廳五頁接上正式遊戲資料、狀態、素材、YouTube 連結與 DEMO API。
7. 將遊戲官網三頁接上正式內容、圖片、四語系與發布版本流程。
8. 依通知規格把通知 Placeholder 替換為正式站內通知中心。
