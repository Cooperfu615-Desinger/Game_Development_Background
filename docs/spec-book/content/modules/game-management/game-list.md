...
| 主要來源 | `GAME_LIST_SPEC.md`、`PROVIDER_PORTAL_SPEC.md` |
| 頁面角色 | Provider 遊戲主資料與三環境摘要入口 |

> 本頁是唯讀總覽與跨頁入口，不直接發布版本、調整 RTP／限紅、上傳素材或控制代理商個別開關。原型資料與狀態為 mock；正式遊戲、環境與 GGAP 同步契約仍受集中 TBD 管理。

## 1. 目的與責任邊界

- 顯示 Provider 所有遊戲，包括尚未部署、啟用或上架者。
- 同時比較 Production、DEMO、Test 的版本與狀態，並查看 Production／DEMO RTP 摘要。
- 顯示 Production 營運控制及 GGAP 目錄／Release 同步結果，提供詳情與相關管理頁導流。
- 不建立錢包、代理商、商戶或會員主資料；GGAP 同步不等於代理商個別開放。
- Test 只讀且不得進入正式 Game Round、財務或 Provider 風控。

## 2. 核心資料與環境規則

- `game_id` 是穩定主鍵；名稱、類型、環境版本與狀態不得作 join key。
- Production／DEMO 的狀態 Draft：上架、維護中、未啟用；Test：未部署、部署中、已部署、部署失敗。
- 無生效版本時顯示 `—` 與未啟用；`＋` 只代表有較新已部署 Release，不取代目前生效版號。
- Production 營運控制 Draft：正常、維護中、暫停新局；與發布狀態分開。
- 下架、維護或暫停新局只阻擋新 Launch，不改寫既有 Game Round。

## 3. 六區塊資訊架構

1. 環境摘要：Production 上架、DEMO 上架、需要留意。
2. 查詢條件：主資料、三環境、營運、同步與 RTP 狀態。
3. 結果摘要：命中數、applied filters、排序與唯讀說明。
4. 遊戲寬列表：14 個欄位與兩層群組表頭。
5. RTP Tips 與狀態：理論／實際／偏差／樣本／窗口／更新。
6. 遊戲詳情：三環境、營運、同步及跨頁導流。
...
<figure class="page-anatomy lifecycle-anatomy lifecycle-anatomy--list" aria-label="遊戲列表六區塊畫面示意">
  <div class="lifecycle-anatomy__canvas">
    ...
    <a class="anatomy-zone lifecycle-anatomy__filters" href="#5-查詢與結果摘要" aria-label="前往第二區，查詢條件"><span class="anatomy-zone__number">02</span><strong>查詢條件</strong><i>代號／名稱</i><i>類型</i><i>正式狀態</i><i>DEMO 狀態</i><i>Test 狀態</i><i>營運控制</i><i>GGAP 同步</i><i>RTP 監控</i><b>查詢</b></a>
    ...
    <div class="lifecycle-anatomy__split">
      ...
      <a class="anatomy-zone lifecycle-anatomy__detail" href="#8-遊戲詳情與導流" aria-label="前往第六區，遊戲詳情"><span class="anatomy-zone__number">06</span><small>GAME SUMMARY</small><strong>星際寶藏 / GAME-001</strong><i>Production · DEMO · Test · 營運控制 · GGAP 同步</i></a>
    </div>
  </div>
...
桌機版兩層表頭共 14 欄：遊戲代號、遊戲名稱、類型；Production 版號／狀態／RTP；DEMO 版號／狀態／RTP；Test 版號／狀態；營運控制；GGAP 同步；詳情。

- Production／DEMO 版號是目前生效 Release；Test 是 RD 目前部署版本。
- Production／DEMO 可使用不同 Release，不要求同步。
...
- 詳情 Dialog 顯示三環境版本／狀態、Production／DEMO RTP、營運控制、GGAP 同步與更新時間。
- Production／DEMO 版號、狀態及 `＋` 導向 `/games/environments?game_id=...&environment=...`。
- 營運控制導向 `/games/settings?game_id=...`；數值、版本、素材使用各自 route 與精確 `game_id`。
...
| 遊戲詳情 | 主資料、三環境 snapshot、RTP、營運、同步、allowed links。 |
...
驗收條件：14 欄與兩層表頭完整；三環境不混用；Test 唯讀；RTP Tips 含樣本與窗口；`＋` 不取代生效版；GGAP 同步不等於代理商開放；Loading／empty／error／stale／Forbidden 可驗收；Desktop／Mobile 可操作。
...
正式遊戲 schema、三環境狀態、RTP 資料、API、權限與 deep link 核准並通過驗收後，才可改為 Confirmed。
