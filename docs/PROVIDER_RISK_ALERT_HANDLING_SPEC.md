...
> 版本：0.2.0
> 更新日期：2026-08-11
> 狀態：產品方向已確認，前端完整 mock 原型已完成；正式 API、權限與核准契約待確認
...
- 五張依目前環境與 mock 狀態動態計算的摘要卡；摘要 Tips 顯示實際分類與明細，點擊後套用工作佇列快捷篩選。點擊任一摘要卡會回到 `status: active` 的有效 Alert 範圍，清除會互相衝突的其他條件，只保留目前環境與該張卡 shortcut；再次點擊同一卡片只清除 shortcut，仍維持有效 Alert 範圍。
- 16 欄工作佇列、依規格的逐層優先排序、分頁、載入／空資料／查詢錯誤狀態與完整篩選結果匯出入口。
- `alert_id`／`risk_event_id` query 導入、Risk Event／請求紀錄入口、關聯 Game Round 導向 `/reports`；Alert 詳情的 Risk Event 入口會以完整 `risk_event_id` 導向報表並自動開啟對應事件。
- 大型 Alert 詳情 Dialog、健康檢查、處理時間線與固定操作區；所有操作僅更新前端 mock 狀態，不呼叫隔離、GGAP 通知或正式資料 API。
...
- `alert_id`、`risk_event_id`、遊戲商／GGAP 遊戲局識別碼、`provider_event_id`、API 路徑、錯誤碼、版本號與正式欄位值保持原樣；只有顯示標籤與 mock 敘述翻譯為中文。
