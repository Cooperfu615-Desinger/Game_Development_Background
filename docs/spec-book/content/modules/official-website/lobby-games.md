...
- 只有符合正式納入條件的遊戲出現在本清單；條件與移除流程依 `TBD-DOM-005`。
- 玩家狀態只控制 DEMO CTA；即將開放與維護中仍保留公開卡片。
- 公開數值來自已核准 snapshot，不允許在列表直接編輯。

## 3. 六區塊資訊架構

1. 清單摘要：大廳遊戲、三種狀態及需補資料。
2. 查詢條件：關鍵字、狀態、類型、readiness 與版號。
...
摘要使用完整集合，顯示三種狀態及阻擋公開數量；資料來源失敗不顯示 0。由總覽 deep link 進入時套用 query 並可清除。
...
單筆摘要顯示 lobby revision、四語完整度、素材／數值參照、玩家狀態、最後發布及阻擋／警告。詳情導向 `/lobby/management?gameId=...`；預覽導向 `/lobby/preview?gameId=...&mode=published`。若來源 snapshot 被撤銷或缺失，入口仍可查看原因，不可靜默套用最新草稿。
...
API 提供摘要、filter options、列表與公開 snapshot 摘要；後端強制 Provider scope 並回傳 stable sorting、generated_at、source status 與 allowed links。正式契約依 `TBD-API-006`。

本頁使用 `1500px` 寬版。驗收條件：所有三狀態遊戲都可找到；八欄完整；公開數值可追溯；列表唯讀；大廳狀態不冒充全域／GGAP 開關；Desktop／Mobile、鍵盤與替代狀態可驗收。

## 9. 待確認與 Draft 移除條件

- `TBD-DOM-005`：納入、排序、公開版本與移除模型。
- `TBD-DAT-006`：多語與素材 fallback。
- `TBD-API-006`、`TBD-SEC-001`：清單／摘要 API 與資料 scope。

正式納入條件、公開 snapshot、API、權限及排序／錯誤規則核准後，才可改為 Confirmed。
