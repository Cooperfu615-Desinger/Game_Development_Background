...
const mockEvents: RiskEvent[] = [
    makeEvent({
        riskEventId: 'rsk_01jz4m8v3k6q2d7p9x5n1c0bqa', alertId: 'alt_01jz4m8v3k6q2d7p9x5n1c0bqa', detectedAt: hoursAgo(3, 12), environment: 'production', gameType: 'slots', gameId: 'NH-001', gameName: 'Neon Heist', gameVersion: 'v2.4.1', source: 'game_round', anomalyType: 'settlement_failure', severity: 'critical', affectedRounds: 146, status: 'pending', mitigationStatus: 'failed', mitigationAction: '重試結算 · 暫停新啟動', mitigationScope: 'Neon Heist / v2.4.1 / 正式環境', isolationStatus: 'not_isolated', ggapNotificationStatus: 'failed', occurredCount: 48, isOngoing: true, ruleId: 'risk.settlement.failure-rate', ruleVersion: 'v3.2', threshold: '失敗率 > 3% / 15 分鐘', actualValue: '11.8%（146 / 1,237）', statisticWindow: '近 15 分鐘', errorCode: 'SETTLE_UPSTREAM_502', errorMessage: 'GGAP 結算端點回傳 502', requestSummary: 'POST /settle · 逾時 5 秒 · 重試 2/3', responseSummary: '502 Bad Gateway（閘道錯誤） · 上游服務不可用', mitigationFailureReason: '暫停新啟動權限尚未取得；重試佇列持續失敗。', ggapProviderEventId: 'pev_7d9c1a2e', ggapRetryCount: 3, providerRoundIds: ['pr_nh_8a31f2', 'pr_nh_8a31f3', 'pr_nh_8a31f4'], ggapRoundIds: ['gg_7A31F2', 'gg_7A31F3', 'gg_7A31F4'], relatedRounds: [
        ...
        ], timeline: makeTimeline(hoursAgo(3, 12), [[0, '偵測風控事件', '結算失敗率超過規則門檻。', 'danger'], [4, '自動緩解失敗', '有限重試與新啟動限制均未完成。', 'warning'], [18, 'GGAP 通知失敗', '通知已重試 3 次，尚未取得 ACK。', 'danger']]), mitigationStartedAt: hoursAgo(3, 9), mitigationCompletedAt: null, ggapSentAt: hoursAgo(3, 5), ggapAckAt: null,
    }),
    makeEvent({
        riskEventId: 'rsk_01jz4f6b2c9m7q8x3n5v0d1epa', alertId: 'alt_01jz4f6b2c9m7q8x3n5v0d1epa', detectedAt: hoursAgo(5, 18), environment: 'production', gameType: 'crash', gameId: 'ND-014', gameName: 'Neon Drift', gameVersion: 'v1.8.3', source: 'callback', anomalyType: 'callback_failure', severity: 'high', affectedRounds: 64, status: 'investigating', mitigationStatus: 'applied', mitigationAction: '重新排入回呼可靠佇列', mitigationScope: 'Neon Drift / v1.8.3 / 正式環境', isolationStatus: 'isolated', ggapNotificationStatus: 'acknowledged', occurredCount: 19, isOngoing: false, ruleId: 'risk.callback.missing', ruleVersion: 'v2.6', threshold: '未收到回呼 > 90 秒', actualValue: '64 筆未收到回呼', statisticWindow: '近 30 分鐘', errorCode: 'CALLBACK_ACK_TIMEOUT', errorMessage: '回呼確認超過 90 秒', requestSummary: '回呼傳送佇列 · 批次 4F91', responseSummary: '未收到 ACK · 已接受重試佇列', mitigationFailureReason: '—', ggapProviderEventId: 'pev_4f91c82a', ggapRetryCount: 1, providerRoundIds: ['pr_nd_4f91a2'], ggapRoundIds: ['gg_4F91A2'], relatedRounds: [{ providerId: 'pr_nd_4f91a2', ggapId: 'gg_4F91A2', settleStatus: '回呼重試中', occurredAt: hoursAgo(5, 18) }], timeline: makeTimeline(hoursAgo(5, 18), [[0, '偵測風控事件', '回呼未在等待時間內回覆。', 'danger'], [7, '自動緩解已套用', '失敗回呼已重新排入可靠佇列。', 'success'], [22, '人工接手', '營運人員開始調查受影響遊戲回合。', 'warning']]), mitigationStartedAt: hoursAgo(5, 14), mitigationCompletedAt: hoursAgo(5, 7), ggapSentAt: hoursAgo(5, 1), ggapAckAt: hoursAgo(4, 56),
    }),
    makeEvent({
        riskEventId: 'rsk_01jz4b1p8w4y6h0m2k9d7c3sqa', alertId: 'alt_01jz4b1p8w4y6h0m2k9d7c3sqa', detectedAt: hoursAgo(9, 40), environment: 'production', gameType: 'table', gameId: 'PC-009', gameName: 'Paper Crane', gameVersion: 'v3.1.0', source: 'data_quality', anomalyType: 'duplicate_settlement', severity: 'critical', affectedRounds: 27, status: 'mitigated', mitigationStatus: 'applied', mitigationAction: '依冪等鍵忽略重複結算', mitigationScope: 'Paper Crane / v3.1.0 / 正式環境', isolationStatus: 'isolated', ggapNotificationStatus: 'acknowledged', occurredCount: 9, isOngoing: false, ruleId: 'risk.round.duplicate-settle', ruleVersion: 'v1.9', threshold: '相同遊戲回合 ID 出現 2 次以上結算', actualValue: '27 筆重複請求', statisticWindow: '近 10 分鐘', errorCode: 'ROUND_IDEMPOTENCY_CONFLICT', errorMessage: '重複結算請求遭冪等防護拒絕', requestSummary: 'POST /settle · 重複使用冪等鍵', responseSummary: '409 Conflict（衝突） · 已忽略重複請求', mitigationFailureReason: '—', ggapProviderEventId: 'pev_aa31d99e', ggapRetryCount: 0, providerRoundIds: ['pr_pc_aa31d9', 'pr_pc_aa31da'], ggapRoundIds: ['gg_AA31D9', 'gg_AA31DA'], relatedRounds: [{ providerId: 'pr_pc_aa31d9', ggapId: 'gg_AA31D9', settleStatus: '已防重', occurredAt: hoursAgo(9, 40) }, { providerId: 'pr_pc_aa31da', ggapId: 'gg_AA31DA', settleStatus: '已防重', occurredAt: hoursAgo(9, 34) }], timeline: makeTimeline(hoursAgo(9, 40), [[0, '偵測風控事件', '資料品質規則判定為重複結算。', 'danger'], [5, '自動緩解已套用', '冪等防重擋下重複請求。', 'success'], [31, '事件已緩解', '未再觀察到新的重複結算。', 'success']]), mitigationStartedAt: hoursAgo(9, 37), mitigationCompletedAt: hoursAgo(9, 35), ggapSentAt: hoursAgo(9, 22), ggapAckAt: hoursAgo(9, 20),
    }),
    makeEvent({
        riskEventId: 'rsk_01jz47q3n6p8v2c5m0x1d9beka', detectedAt: hoursAgo(14, 8), environment: 'production', gameType: 'slots', gameId: 'LH-022', gameName: 'Lucky Harbor', gameVersion: 'v5.0.2', source: 'ggap_request', anomalyType: 'request_timeout', severity: 'medium', affectedRounds: 18, status: 'closed', mitigationStatus: 'applied', mitigationAction: '可重試請求退避重試', mitigationScope: 'Lucky Harbor / v5.0.2 / 正式環境', isolationStatus: 'released', ggapNotificationStatus: 'sent', occurredCount: 4, isOngoing: false, ruleId: 'risk.request.timeout', ruleVersion: 'v2.2', threshold: '回應時間 > 5,000 ms', actualValue: 'P95 7,240 ms', statisticWindow: '近 15 分鐘', errorCode: 'UPSTREAM_TIMEOUT', errorMessage: 'GGAP 回應超過逾時門檻', requestSummary: 'POST /launch · 逾時 5 秒 · 重試 1/3', responseSummary: '重試成功 · 200 OK（成功）', mitigationFailureReason: '—', ggapProviderEventId: 'pev_1c0b83f4', ggapRetryCount: 1, providerRoundIds: ['pr_lh_1c0b83'], ggapRoundIds: ['gg_1C0B83'], relatedRounds: [{ providerId: 'pr_lh_1c0b83', ggapId: 'gg_1C0B83', settleStatus: '已完成', occurredAt: hoursAgo(14, 8) }], timeline: makeTimeline(hoursAgo(14, 8), [[0, '偵測風控事件', '請求超過逾時門檻。', 'warning'], [6, '自動緩解已套用', '退避重試成功，服務恢復。', 'success'], [86, '事件已關閉', '確認無持續影響後完成結案。', 'neutral']]), mitigationStartedAt: hoursAgo(14, 4), mitigationCompletedAt: hoursAgo(14, 2), ggapSentAt: hoursAgo(13, 55), ggapAckAt: null,
    }),
    makeEvent({
        riskEventId: 'rsk_01jz45k2a9d6m3p7x8v0c1bqea', detectedAt: hoursAgo(17, 50), environment: 'production', gameType: 'crash', gameId: 'OR-006', gameName: 'Orbit Rush', gameVersion: 'v1.4.8', source: 'data_quality', anomalyType: 'data_missing', severity: 'low', affectedRounds: 3, status: 'false_positive', mitigationStatus: 'not_required', mitigationAction: '不需處理', mitigationScope: 'Orbit Rush / v1.4.8 / 正式環境', isolationStatus: 'not_isolated', ggapNotificationStatus: 'not_required', occurredCount: 3, isOngoing: false, ruleId: 'risk.payload.required-fields', ruleVersion: 'v1.4', threshold: '必要欄位缺失 >= 1', actualValue: '3 筆（已由延遲資料補齊）', statisticWindow: '近 30 分鐘', errorCode: 'PAYLOAD_FIELD_DELAYED', errorMessage: '必要欄位在初次驗證後才到達', requestSummary: '回呼內容 · 延遲欄位補齊', responseSummary: '結算前已完成欄位補齊', mitigationFailureReason: '—', ggapProviderEventId: '—', ggapRetryCount: 0, providerRoundIds: ['pr_or_2c91a0'], ggapRoundIds: ['gg_2C91A0'], relatedRounds: [{ providerId: 'pr_or_2c91a0', ggapId: 'gg_2C91A0', settleStatus: '已完成', occurredAt: hoursAgo(17, 50) }], timeline: makeTimeline(hoursAgo(17, 50), [[0, '偵測風控事件', '結構驗證器暫時標記欄位缺失。', 'warning'], [12, '資料補齊', '延遲欄位到達，遊戲回合狀態正常。', 'success'], [62, '確認誤報', '確認不構成實際資料異常。', 'neutral']]), mitigationStartedAt: null, mitigationCompletedAt: null, ggapSentAt: null, ggapAckAt: null,
    }),
    makeEvent({
        riskEventId: 'rsk_01jz42n8w5q1d7c3m9x6v0bpea', alertId: 'alt_01jz42n8w5q1d7c3m9x6v0bpea', detectedAt: hoursAgo(21, 22), environment: 'production', gameType: 'slots', gameId: 'SG-031', gameName: 'Solar Garden', gameVersion: 'v2.0.0', source: 'game_service', anomalyType: 'service_unavailable', severity: 'high', affectedRounds: 91, status: 'pending', mitigationStatus: 'failed', mitigationAction: '暫停指定版本新啟動', mitigationScope: 'Solar Garden / v2.0.0 / 正式環境', isolationStatus: 'not_isolated', ggapNotificationStatus: 'failed', occurredCount: 22, isOngoing: true, ruleId: 'risk.game-service.health', ruleVersion: 'v4.1', threshold: '健康檢查連續失敗 3 次', actualValue: '連續失敗 7 次', statisticWindow: '近 8 分鐘', errorCode: 'GAME_SERVICE_503', errorMessage: '遊戲服務無法使用', requestSummary: 'GET /health · 間隔 30 秒', responseSummary: '503 Service Unavailable（服務不可用）', mitigationFailureReason: '版本隔離執行器尚未取得有效鎖。', ggapProviderEventId: 'pev_98c20b7a', ggapRetryCount: 2, providerRoundIds: ['pr_sg_98c20b', 'pr_sg_98c21'], ggapRoundIds: ['gg_98C20B', 'gg_98C21'], relatedRounds: [{ providerId: 'pr_sg_98c20b', ggapId: 'gg_98C20B', settleStatus: '等待服務恢復', occurredAt: hoursAgo(21, 22) }, { providerId: 'pr_sg_98c21', ggapId: 'gg_98C21', settleStatus: '回呼保留', occurredAt: hoursAgo(21, 15) }], timeline: makeTimeline(hoursAgo(21, 22), [[0, '偵測風控事件', '健康檢查連續失敗，事件升級為高。', 'danger'], [3, '自動緩解失敗', '版本隔離未成功套用。', 'warning'], [16, 'GGAP 通知失敗', '通知服務等待重試。', 'danger']]), mitigationStartedAt: hoursAgo(21, 18), mitigationCompletedAt: null, ggapSentAt: hoursAgo(21, 5), ggapAckAt: null,
    }),
    makeEvent({
        riskEventId: 'rsk_01jz3y7m4c8p1d6v0x9q2bnea', detectedAt: hoursAgo(31, 10), environment: 'production', gameType: 'table', gameId: 'FM-017', gameName: 'Fortune Mahjong', gameVersion: 'v4.2.2', source: 'game_math', anomalyType: 'payout_anomaly', severity: 'medium', affectedRounds: 8, status: 'investigating', mitigationStatus: 'pending', mitigationAction: '重新計算統計窗口', mitigationScope: 'Fortune Mahjong / v4.2.2 / 正式環境', isolationStatus: 'not_isolated', ggapNotificationStatus: 'pending', occurredCount: 2, isOngoing: false, ruleId: 'risk.math.payout-spike', ruleVersion: 'v1.7', threshold: '單局派彩 > 限紅 4 倍', actualValue: '最高 5.6 倍', statisticWindow: '近 60 分鐘', errorCode: 'PAYOUT_OUTLIER', errorMessage: '派彩超過已設定範圍', requestSummary: '遊戲數值監控 · 樣本 8,412', responseSummary: '等待規則檢視', mitigationFailureReason: '—', ggapProviderEventId: 'pev_1a3c70d8', ggapRetryCount: 0, providerRoundIds: ['pr_fm_1a3c70'], ggapRoundIds: ['gg_1A3C70'], relatedRounds: [{ providerId: 'pr_fm_1a3c70', ggapId: 'gg_1A3C70', settleStatus: '已完成，待調查', occurredAt: hoursAgo(31, 10) }], timeline: makeTimeline(hoursAgo(31, 10), [[0, '偵測風控事件', '數值規則偵測到派彩離群。', 'warning'], [23, '人工接手', '營運人員開始檢視規則版本與樣本量。', 'neutral']]), mitigationStartedAt: hoursAgo(31, 2), mitigationCompletedAt: null, ggapSentAt: hoursAgo(30, 48), ggapAckAt: null,
    }),
    makeEvent({
        riskEventId: 'rsk_01jz3s4v8n2d5c0m7x9q1bpea', alertId: 'alt_01jz3s4v8n2d5c0m7x9q1bpea', detectedAt: hoursAgo(8, 6), environment: 'demo', gameType: 'slots', gameId: 'NH-001', gameName: 'Neon Heist', gameVersion: 'v2.5.0-rc1', source: 'ggap_request', anomalyType: 'request_timeout', severity: 'high', affectedRounds: 12, status: 'pending', mitigationStatus: 'applied', mitigationAction: '展示環境啟動重新排隊', mitigationScope: 'Neon Heist / v2.5.0-rc1 / 展示環境', isolationStatus: 'isolated', ggapNotificationStatus: 'sent', occurredCount: 5, isOngoing: false, ruleId: 'risk.request.timeout', ruleVersion: 'v2.2', threshold: '回應時間 > 5,000 ms', actualValue: 'P95 8,140 ms', statisticWindow: '近 30 分鐘', errorCode: 'DEMO_UPSTREAM_TIMEOUT', errorMessage: '展示環境上游回應超過門檻', requestSummary: 'POST /launch · 展示環境端點', responseSummary: '重試成功 · 200 OK（成功）', mitigationFailureReason: '—', ggapProviderEventId: 'pev_5b1e092a', ggapRetryCount: 1, providerRoundIds: ['pr_nh_demo_5b1e'], ggapRoundIds: ['gg_DEMO5B1E'], relatedRounds: [{ providerId: 'pr_nh_demo_5b1e', ggapId: 'gg_DEMO5B1E', settleStatus: '已完成', occurredAt: hoursAgo(8, 6) }], timeline: makeTimeline(hoursAgo(8, 6), [[0, '偵測風控事件', '展示環境請求超過逾時門檻。', 'warning'], [8, '自動緩解已套用', '請求已重新排隊並成功完成。', 'success']]), mitigationStartedAt: hoursAgo(8, 4), mitigationCompletedAt: hoursAgo(8, 1), ggapSentAt: hoursAgo(8), ggapAckAt: null,
    }),
    makeEvent({
        riskEventId: 'rsk_01jz3n2p6c8v4d0m9x1q7besa', detectedAt: hoursAgo(76, 30), environment: 'demo', gameType: 'crash', gameId: 'OR-006', gameName: 'Orbit Rush', gameVersion: 'v1.9.0-demo', source: 'callback', anomalyType: 'callback_latency', severity: 'medium', affectedRounds: 7, status: 'closed', mitigationStatus: 'applied', mitigationAction: '回呼退避重試', mitigationScope: 'Orbit Rush / v1.9.0-demo / 展示環境', isolationStatus: 'released', ggapNotificationStatus: 'acknowledged', occurredCount: 3, isOngoing: false, ruleId: 'risk.callback.latency', ruleVersion: 'v2.1', threshold: 'P95 > 1,500 ms', actualValue: 'P95 2,140 ms', statisticWindow: '近 20 分鐘', errorCode: 'CALLBACK_LATENCY_HIGH', errorMessage: '回呼回應延遲超過門檻', requestSummary: '回呼傳送佇列 · 展示環境批次', responseSummary: '重試後收到 ACK', mitigationFailureReason: '—', ggapProviderEventId: 'pev_8b30a1c2', ggapRetryCount: 1, providerRoundIds: ['pr_or_demo_8b30'], ggapRoundIds: ['gg_DEMO8B30'], relatedRounds: [{ providerId: 'pr_or_demo_8b30', ggapId: 'gg_DEMO8B30', settleStatus: '已完成', occurredAt: hoursAgo(76, 30) }], timeline: makeTimeline(hoursAgo(76, 30), [[0, '偵測風控事件', '展示環境回呼延遲超過門檻。', 'warning'], [16, '自動緩解已套用', '退避重試成功並取得 ACK。', 'success'], [102, '事件已關閉', '完成唯讀檢視。', 'neutral']]), mitigationStartedAt: hoursAgo(76, 24), mitigationCompletedAt: hoursAgo(76, 18), ggapSentAt: hoursAgo(76, 10), ggapAckAt: hoursAgo(76, 8),
    }),
    makeEvent({
        riskEventId: 'rsk_01jz2k8d4v6p1c0m9x3q7bnea', detectedAt: hoursAgo(126), environment: 'production', gameType: 'slots', gameId: 'CV-001', gameName: 'Celestial Vault', gameVersion: 'v6.0.1', source: 'data_quality', anomalyType: 'round_link_failure', severity: 'low', affectedRounds: 2, status: 'closed', mitigationStatus: 'not_required', mitigationAction: '不需處理', mitigationScope: 'Celestial Vault / v6.0.1 / 正式環境', isolationStatus: 'not_isolated', ggapNotificationStatus: 'not_required', occurredCount: 2, isOngoing: false, ruleId: 'risk.round.link', ruleVersion: 'v1.3', threshold: '遊戲回合關聯失敗 >= 1', actualValue: '2 筆待補資料', statisticWindow: '近 1 小時', errorCode: 'ROUND_LINK_PENDING', errorMessage: '遊戲回合關聯暫時無法使用', requestSummary: '遊戲回合對帳工作程序', responseSummary: '關閉前已恢復關聯', mitigationFailureReason: '—', ggapProviderEventId: '—', ggapRetryCount: 0, providerRoundIds: ['pr_cv_91a2'], ggapRoundIds: ['gg_91A2'], relatedRounds: [{ providerId: 'pr_cv_91a2', ggapId: 'gg_91A2', settleStatus: '已完成', occurredAt: hoursAgo(126) }], timeline: makeTimeline(hoursAgo(126), [[0, '偵測風控事件', '遊戲回合關聯暫時失敗。', 'warning'], [46, '事件已關閉', '關聯資料恢復且完成檢視。', 'neutral']]), mitigationStartedAt: null, mitigationCompletedAt: null, ggapSentAt: null, ggapAckAt: null,
    }),
]
...
function goToAlerts(event: RiskEvent) {
    const alertId = event.alertId?.trim()
    const riskEventId = event.riskEventId.trim()
    if (!alertId || !riskEventId) return
    void router.push({ path: '/monitoring/alerts', query: { alert_id: alertId, risk_event_id: riskEventId } })
}
    ...
    <div class="risk-reports-page page-stack">
        ...
        <FilterCard v-if="!queryError" title="查詢條件" description="所有摘要、待關注異常與風控事件列表均依目前套用的查詢條件同步更新。">
            <template #default>
                ...
                <div v-if="advancedVisible" class="risk-report-filter-grid risk-report-filter-grid--advanced">
                    ...
                    <div class="field"><label for="risk-provider-round">遊戲商遊戲回合 ID（Provider Game Round ID） <small>精確查詢</small></label><InputText id="risk-provider-round" v-model="draftFilters.providerGameRoundId" placeholder="輸入完整回合 ID" fluid @keyup.enter="applyFilters" /></div>
                    <div class="field"><label for="risk-ggap-round">GGAP 遊戲回合 ID（GGAP Round ID） <small>精確查詢</small></label><InputText id="risk-ggap-round" v-model="draftFilters.ggapRoundId" placeholder="輸入完整回合 ID" fluid @keyup.enter="applyFilters" /></div>
                </div>
            </template>
        </FilterCard>
        ...
        <section v-if="!queryError" class="risk-report-list-section" aria-labelledby="risk-list-title">
            ...
            <SectionCard class="risk-report-table-card">
                ...
                <DataTable v-else class="risk-report-data-table" :value="matchingRows" data-key="riskEventId" :loading="loading" scrollable paginator :first="first" :rows="8" :rows-per-page-options="[8, 16, 32]" table-style="min-width: 2180px" paginator-template="FirstPageLink PrevPageLink CurrentPageReport NextPageLink LastPageLink RowsPerPageDropdown" current-page-report-template="{first}-{last} / {totalRecords}" @page="handlePage">
                    ...
                    <Column header="操作" frozen align-frozen="right" style="width: 166px; min-width: 166px"><template #body="{ data }"><div class="risk-report-row-actions"><Button type="button" icon="pi pi-eye" text rounded severity="secondary" aria-label="查看風控事件詳情" data-testid="risk-details-button" @click="openDetails(data)" /><Button v-if="data.alertId" type="button" icon="pi pi-external-link" text rounded severity="danger" aria-label="前往告警處理" @click="goToAlerts(data)" /></div></template></Column>
                </DataTable>
            </SectionCard>
        </section>
        ...
        <Dialog v-model:visible="exportVisible" modal dismissable-mask header="匯出風控報表" class="risk-export-dialog" :style="{ width: 'min(560px, calc(100vw - 32px))' }">
            <div class="risk-export-content">
                ...
                <div class="risk-export-mock-note"><i class="pi pi-info-circle" /><span>原型／模擬資料：目前只呈現匯出入口與格式選擇，不產生實體檔案。</span></div>
            </div>
        ...
        <Dialog v-model:visible="detailsVisible" modal dismissable-mask class="risk-detail-dialog" :style="{ width: 'min(1180px, calc(100vw - 32px))' }" :header="selectedEvent ? `${selectedEvent.riskEventId} / 風控事件詳情` : '風控事件詳情'">
            <div v-if="selectedEvent" class="risk-detail-content">
                ...
                <section class="risk-detail-section"><div class="risk-detail-section-title"><i class="pi pi-code" /><h3>請求／回應摘要</h3></div><div class="risk-detail-code-grid"><div><span>請求（Request）</span><code>{{ selectedEvent.requestSummary }}</code></div><div><span>回應（Response）</span><code>{{ selectedEvent.responseSummary }}</code></div><div class="risk-detail-error"><span>錯誤訊息（Error message）</span><code>{{ selectedEvent.errorMessage }}</code></div></div></section>
