...
import type { AlertAction, AlertResolutionCode, CommandOutcome, ProviderEnvironment, RiskAlert, RiskSeverity } from '@/types/providerRisk'
...
const {
    getRiskEvent, latestJobAttempts, actionDisabledReason, closeGateFailures, applyAlertCommand, refresh,
} = useProviderRiskMock()
const rowsPerPage = 8
const page = ref(1)
...
const defaults: AlertFilter = { environment: 'production', status: '', severity: '', assignee: '', game: '', alertId: '', riskEventId: '', jobStatus: '', actualState: '', deliveryStatus: '' }
const draft = reactive<AlertFilter>({ ...defaults })
const applied = ref<AlertFilter>({ ...defaults })
...
const filteredAlerts = computed(() => providerRiskState.alerts
    .filter((alert) => alert.environment === applied.value.environment)
    .filter((alert) => !applied.value.status || alert.status === applied.value.status)
    .filter((alert) => !applied.value.severity || alert.severity === applied.value.severity)
    .filter((alert) => !applied.value.assignee || `${alert.assigneeName ?? ''} ${alert.waitingReason ?? ''}`.toLowerCase().includes(applied.value.assignee.toLowerCase()))
    .filter((alert) => {
        const event = getRiskEvent(alert.riskEventId)
        return !applied.value.game || `${event?.gameId ?? ''} ${event?.gameName ?? ''} ${event?.gameVersion ?? ''}`.toLowerCase().includes(applied.value.game.toLowerCase())
    })
    .filter((alert) => !applied.value.alertId || alert.alertId.toLowerCase().includes(applied.value.alertId.toLowerCase()))
    .filter((alert) => !applied.value.riskEventId || alert.riskEventId.toLowerCase().includes(applied.value.riskEventId.toLowerCase()))
    .filter((alert) => !applied.value.jobStatus || alert.mitigationJobs.some((job) => job.status === applied.value.jobStatus))
    .filter((alert) => !applied.value.actualState || alert.isolation?.actualState === applied.value.actualState)
    .filter((alert) => !applied.value.deliveryStatus || alert.deliveries.some((delivery) => delivery.status === applied.value.deliveryStatus))
...
const activeEnvironmentAlerts = computed(() => providerRiskState.alerts.filter((alert) => alert.environment === applied.value.environment && alert.status !== 'closed'))
const summaries = computed(() => {
    const alerts = activeEnvironmentAlerts.value
    return [
        { key: 'unassigned', label: '未指派', value: alerts.filter((alert) => !alert.assigneeId).length, note: 'New 與處理中都需明確 owner' },
        { key: 'high', label: '高風險', value: alerts.filter((alert) => ['critical', 'high'].includes(alert.severity)).length, note: 'High + Critical active Alert' },
        { key: 'isolated', label: '實際隔離中', value: alerts.filter((alert) => alert.isolation?.actualState === 'isolated').length, note: '以 actual_state 計算' },
        { key: 'failed', label: 'Job／Delivery 失敗', value: alerts.filter((alert) => latestJobAttempts(alert).some((job) => job.status === 'failed') || alert.deliveries.at(-1)?.status === 'failed').length, note: '以最新 attempt／必要投遞計算' },
        { key: 'overdue', label: '逾期覆核', value: alerts.filter((alert) => alert.reviewDueAt && alert.reviewDueAt < new Date()).length, note: 'review_due_at 已逾期' },
    ]
})
...
function resetFilters() { Object.assign(draft, defaults, { environment: applied.value.environment }); applyFilters() }
function activateSummary(key: string) {
    Object.assign(draft, { status: '', severity: '', assignee: '', jobStatus: '', actualState: '', deliveryStatus: '' })
    if (key === 'unassigned') draft.assignee = '等待'
    if (key === 'high') draft.severity = 'high'
    if (key === 'isolated') draft.actualState = 'isolated'
    if (key === 'failed') draft.jobStatus = 'failed'
    applyFilters()
}
...
function exportCsv() {
    exportToCSV(filteredAlerts.value.map((alert) => {
        const event = getRiskEvent(alert.riskEventId)
        return { alertId: alert.alertId, riskEventId: alert.riskEventId, environment: alert.environment, severity: alert.severity, game: `${event?.gameName ?? ''} ${event?.gameVersion ?? ''}`, eventStatus: event?.status ?? '', alertStatus: alert.status, assignee: alert.assigneeName ?? '', waiting: alert.waitingReason ?? '', job: alert.mitigationJobs.at(-1)?.status ?? '', desired: alert.isolation?.desiredState ?? '', actual: alert.isolation?.actualState ?? '', delivery: alert.deliveries.at(-1)?.status ?? '', due: alert.reviewDueAt?.toISOString() ?? '' }
    }), `provider-risk-alerts-${applied.value.environment}`, { alertId: 'Alert ID', riskEventId: 'Risk Event ID', environment: '環境', severity: '嚴重度', game: '遊戲／版本', eventStatus: 'Event 狀態', alertStatus: 'Alert 狀態', assignee: '負責人', waiting: '等待原因', job: 'Job', desired: '隔離期望', actual: '隔離實際', delivery: 'GGAP Delivery', due: '覆核期限' })
...
<template>
    <main class="rc-page rc-page--fluid rc-stack">
        ...
        <section class="rc-summary-grid"><button v-for="item in summaries" :key="item.key" class="rc-card rc-summary" @click="activateSummary(item.key)"><span class="rc-summary-label">{{ item.label }}</span><div class="rc-summary-value">{{ item.value }}</div><div class="rc-summary-note">{{ item.note }}</div></button></section>
        ...
        <section class="rc-card"><div class="rc-card-body rc-stack"><div class="rc-section-head"><div><p class="rc-eyebrow">Queue filters</p><h2>告警工作佇列</h2></div><button class="rc-link" @click="advancedVisible = !advancedVisible">{{ advancedVisible ? '收合進階條件' : '展開進階條件' }}</button></div><div class="rc-filters">
            <label class="rc-label">Alert 狀態<select v-model="draft.status" class="rc-select"><option value="">全部</option><option v-for="(_, key) in alertStatusLabels" :key="key" :value="key">{{ alertStatusLabels[key] }}</option></select></label>
            ...
            <label class="rc-label">負責人／等待原因<input v-model="draft.assignee" class="rc-input" placeholder="人員或 waiting reason"></label>
            <label class="rc-label">遊戲／版本<input v-model="draft.game" class="rc-input" placeholder="名稱、ID 或版本"></label>
        ...
        </div><div v-if="advancedVisible" class="rc-filters"><label class="rc-label">Job 狀態<select v-model="draft.jobStatus" class="rc-select"><option value="">全部</option><option v-for="value in ['queued','running','succeeded','failed','cancelled']" :key="value">{{ value }}</option></select></label><label class="rc-label">隔離 actual<select v-model="draft.actualState" class="rc-select"><option value="">全部</option><option v-for="value in ['not_isolated','applying','isolated','releasing','failed']" :key="value">{{ value }}</option></select></label><label class="rc-label">Delivery<select v-model="draft.deliveryStatus" class="rc-select"><option value="">全部</option><option v-for="value in ['pending','sending','sent','acknowledged','failed']" :key="value">{{ value }}</option></select></label></div><div class="rc-actions"><button class="rc-button rc-button--primary" @click="applyFilters">套用篩選</button><button class="rc-button" @click="resetFilters">重設</button></div></div>
            <div class="rc-table-wrap"><table class="rc-table"><thead><tr><th>嚴重度</th><th>Alert ID</th><th>Risk Event／狀態</th><th>建立時間</th><th>環境</th><th>遊戲／版本</th><th>異常</th><th>影響回合</th><th>Alert 狀態</th><th>負責人／等待</th><th>Job</th><th>隔離 desired／actual</th><th>Delivery</th><th>覆核期限</th><th>版本</th><th>操作</th></tr></thead><tbody>
                <tr v-for="alert in pagedAlerts" :key="alert.alertId" @click="openDetail(alert)"><td><span :class="['rc-pill', `rc-pill--${alert.severity}`]">{{ severityLabels[alert.severity] }}</span></td><td><code>{{ alert.alertId }}</code></td><td><code>{{ alert.riskEventId }}</code><span v-if="getRiskEvent(alert.riskEventId)" class="rc-secondary">{{ riskEventStatusLabels[getRiskEvent(alert.riskEventId)!.status] }}</span></td><td>{{ formatDate(alert.createdAt) }}</td><td>{{ environmentLabels[alert.environment] }}</td><td><span class="rc-primary">{{ getRiskEvent(alert.riskEventId)?.gameName }}</span><span class="rc-secondary">{{ getRiskEvent(alert.riskEventId)?.gameId }} · {{ getRiskEvent(alert.riskEventId)?.gameVersion }}</span></td><td>{{ anomalyLabels[getRiskEvent(alert.riskEventId)?.anomalyType ?? ''] }}<span class="rc-secondary">{{ sourceLabels[getRiskEvent(alert.riskEventId)!.source] }}</span></td><td>{{ getRiskEvent(alert.riskEventId)?.affectedRounds ?? '—' }}</td><td><span :class="['rc-pill', `rc-pill--${alert.status}`]">{{ alertStatusLabels[alert.status] }}</span></td><td>{{ display(alert.assigneeName) }}<span class="rc-secondary">{{ display(alert.waitingReason) }}</span></td><td><span v-if="alert.mitigationJobs.at(-1)" :class="['rc-pill', `rc-pill--${alert.mitigationJobs.at(-1)?.status}`]">{{ alert.mitigationJobs.at(-1)?.status }}</span><span class="rc-secondary">{{ display(alert.mitigationJobs.at(-1)?.mitigationJobId) }}</span></td><td><span class="rc-secondary">D {{ display(alert.isolation?.desiredState) }}</span><span class="rc-secondary">A {{ display(alert.isolation?.actualState) }}</span></td><td><span v-if="alert.deliveries.at(-1)" :class="['rc-pill', `rc-pill--${alert.deliveries.at(-1)?.status}`]">{{ alert.deliveries.at(-1)?.status }}</span><span v-else>—</span></td><td :style="{ color: isOverdue(alert) ? '#b42318' : '' }">{{ formatDate(alert.reviewDueAt) }}</td><td>v{{ alert.version }}</td><td><button class="rc-button" @click.stop="openDetail(alert)">處理</button></td></tr>
