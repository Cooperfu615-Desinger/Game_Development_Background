...
| 類型 | 數量 | 定義 |
|---|---:|---|
| 已有內容原型 | 25 | 已具備主要畫面內容；不代表正式 API 或權限完成 |
| Placeholder | 7 | 已有 route 與延後原因；主要規格或內容待整理 |
...
「環境與發布」仍列為 Active，內容原型已完成 Production／DEMO／Test、遊戲全域上架／下架、版本與素材發布、發布前驗證，以及發布狀態／歷程。所有操作目前只使用前端 mock；GGAP 同步、各代理商個別開放、跨系統重試與正式整合契約只標示責任邊界與依賴，等待 Backend／GGAP 證據後補充，不得自行推定。
...
- 環境與發布：已完成三環境矩陣、發布組合、佇列、preflight、快速／高風險通道、回復及歷程內容原型；正式副作用仍待 Backend／CI/CD。
